/**********************************************************************
//exoskeletonControl.cpp
//Date:2020.8.9
//Descripotion:This is a controller for an exoskeleton device, we control 5 servo motors to
//track the trajectory of shoulder joint.
...
*/
  ...
  for (uint8_t index = 0; index < id_cnt; index++)
  {
      ...
      dynamixel_current[index] = pidController(goal_pos[index],id_array[index]); // 此时的控制频率和写频率一样
      // dynamixel_current[index] = testController(id_array[index]);
      //ROS_INFO("id:%d ,goal position:%d  index:%d",id_array[index],goal_pos[index],index);
...
int MyDynamixelController::testController(int id)
{
  uint8_t id_array[dynamixel_.size()];
  uint8_t id_cnt = 0;
  int index = 0;
