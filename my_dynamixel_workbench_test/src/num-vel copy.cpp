...
int main(int argc,char **argv)
{
	//////////////////////////	  parameters    //////////////////////////
	int eps_max  = 10;
	int step_max = 100;		int step = 0;
	int update_step =  5;
	double gam = 0.95;
	double Q1 = 100;		double Q2 = 1;		
	double Qf = 1;			double Qu = 50;
	...
	K(0,0)=-2; K(0,1)=-0; K(0,2)=0;		Kac(0,0)=-3; Kac(0,1)=0; Kac(0,2)=-1;
  ...
  std::ofstream StatesRecoder("statusRecoder.txt");
  std::ofstream KRecoder("kRecoder.txt");
  ...
  for (int eps = 0; eps < eps_max; eps++) 
	{		// episode
	 ...
	 dxl_wb.itemWrite(ID,"Goal_Position",1500);
	// dxl_wb.itemWrite(ID,"Goal_Position",3000);
	 sleep(2);
	 dxl_wb.itemRead(ID,"Present_Position",&position_data);
	...
	P =MatrixXd::Ones(16,16)*1000;
	 while (step < (step_max - 5) && ros::ok() && check ) 
		{	//   step
		 ...
		 statePre = state;
		 uMat = K*state;
		//  u        = uMat(0,0)+5*(eps_max-eps)/eps_max*float(rand()%1000-500)/500.0;			goal_current = int32_t(u+0.5);
		 u        = uMat(0,0)+100*float(rand()%1000-500)/500.0;			goal_current = int32_t(u+0.5);
		...
		//  state(0,0) = state(0,0) - 4096/2 ;
		 ...
		 //////////////////////////	  更新agent
		temp(0,0)= u*Qu*u;
		costMat = state.transpose()*Qx*state + temp; 	cost = costMat(0,0);  

		xuPre(0,0) = statePre(0,0);	xu(0,0) = state(0,0);
		xuPre(1,0) = statePre(1,0);	xu(1,0) = state(1,0);
		xuPre(2,0) = statePre(2,0);	xu(2,0) = f;
		xuPre(3,0) = u; 			xu(3,0) = (K*state)(0,0);
		...
		//  index_Jum = 2*0.05/(1+gradient.norm()*0.05);
		 index_Jum = 0.001;
		...
		// std::cout << "state:     " << state(0,0) << " " << state(1,0) << " " << state(2,0) << " " << std::endl;
		 ...
		 std::cout << "K    :   " << K  <<endl;
         // save data state
         StatesRecoder << position_data << ";   " << velocity_data << ";   " << goal_current << ";   " <<u<<";	"<< std::endl;
	     KRecoder      << K(0,0) 		   << ";   " << K(0,1) 		  << ";   " << K(0,2) 		<< ";   " << std::endl;
		}
		//////////////////////////	Q学习更新部分, 和AC学习二选一
		// K=H21+Kac*H22;
		// std::cout << "K_before    :   " << K  <<endl;
		// K=H21+K*H22;
		// K=-H21/H22;
		// std::cout << "K    :   " << K  <<endl;
		// std::cout << "H21:       " << H21              << std::endl;
