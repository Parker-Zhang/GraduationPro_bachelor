...
int main(int argc,char **argv)
{
  ...
  std::ofstream StatesRecoder("K_vel_statusRecoder1.txt");
  std::ofstream KRecoder("K_vel_kRecoder1.txt");
	...
	{		// episode
	 ...
	 while (step < (step_max - 5) && ros::ok() && check ) 
		{	//   step
		 ...
		 statePre = state;
		 uMat = K*state;
		 u        = uMat(0,0);			goal_current = int32_t(u*5);
		 ...
		 if(goal_current>limit_current)
			...
			// ROS_INFO("Goal_Velocity:%d",goal_current);
		...
		xuPre(3,0) = u; 			xu(3,0) = (K*state)(0,0)*5;
