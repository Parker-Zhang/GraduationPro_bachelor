...
int MyDynamixelController::pidController(int goal_position_ , int id)
{
  ...
  last_pos_err[index] = pos_com_err[index];
  int32_t pos_com_err_temp = 0;
  for (int index_=0;index_<id_cnt;index_++)
  {
  ...
  pos_com_err_integral[index] += pos_com_err_temp;
  //ROS_INFO("id:%d   pos_com_err:%d   pos_com_err_integral:%d",id,pos_com_err_temp,pos_com_err_integral[index]);
  goal_current += k_com_p*pos_com_err_temp+k_com_i*pos_com_err_integral[index]+k_com_d*(pos_com_err[index]-last_pos_com_err[index]);
...
{
    ros::init(argc,argv,"distributeController");
...
/*
    ...
    std::string folder = node_handle.param<std::string>("outputFilePath", "");
    ...
    { 
      ...
      fout1 << "   position:" << state_msg.present_radian ;
